...
class TestTokenGeneration:
    ...
    def test_bigram_token_generation(self, model_size: str) -> None:
        completed_process = subprocess.run(
            f"python src/model/generate.py --model bigram --size {model_size} --device cpu --max-new-tokens 10".split(),
        )
    ...
    def test_gpt_token_generation(self, model_size: str) -> None:
        completed_process = subprocess.run(
            f"python src/model/generate.py --model gpt --size {model_size} --device cpu --max-new-tokens 10".split(),
        )
        assert completed_process.returncode == 0
